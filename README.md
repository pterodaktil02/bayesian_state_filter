...
Исправлена неидентифицируемость общего смещения калибровок `bias`. Попарная
калибровка определяет только разности между источниками, поэтому без отдельного
условия все `bias` могли медленно уехать на одну и ту же константу и сдвинуть
абсолютный уровень ensemble, не изменив ни одной межсенсорной разности.

Теперь после startup- и live-калибровки явно поддерживается gauge:

```text
median(bias_i) = 0
```

Это согласовано с робастным median-fusion: относительные поправки между
датчиками сохраняются, но общий ноль системы больше не может самопроизвольно
дрейфовать. Старые checkpoint 0.3.1 мигрируются без полного обучения: общий
bias-offset удаляется, а сохраненный уровень и level-history сдвигаются на ту
же величину.
