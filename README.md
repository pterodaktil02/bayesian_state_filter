...
Главное изменение 0.4.0 - исправление тяжелого hot path в online-калибровке
источников. Раньше после каждого нового измерения выполнялась переоценка
`bias/sigma` по растущему окну истории. Для быстрых датчиков это превращало
почти неизменную метрологическую оценку в дорогую операцию на каждом sample и
могло со временем занять весь доступный Home Assistant event-loop CPU.

Теперь каждый sample выполняет только обычное обновление фильтра и дешевый
O(1) drift-monitor. Полная O(history) калибровка запускается адаптивно:

```text
stable   -> примерно раз в calibration_window / 2
watch    -> чаще при заметном дрейфе
unstable -> еще чаще при явной деградации
```

Drift оценивается относительно собственного baseline источника, поэтому датчик
с нормальным историческим уровнем выбросов, например 4%, не считается
нестабильным только из-за того, что он не укладывается в общий фиксированный
порог.

Состояние adaptive scheduler сохраняется в checkpoint. Обычный рестарт с
совместимым checkpoint не запускает полную многосуточную калибровку заново и
не провоцирует немедленный heavy refit.

Кроме того, `source_health` сокращен до оперативных полей, чтобы не
сериализовать большую диагностическую структуру при каждом обновлении sensor.
Подробная CPU-диагностика вынесена в атрибуты `cpu_diag_*`.

В 0.4.0 также добавлен `bias_anchor`:

- `median` - прежний робастный нулевой gauge;
- `mean` - линейный sum-to-zero gauge;
- `passport` - абсолютная привязка по паспортной точности моделей, причем
  каждая модель дает один робастный голос независимо от числа одинаковых
  физических датчиков.
